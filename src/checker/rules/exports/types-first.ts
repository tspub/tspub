...
export const typesFirstRule: Rule = {
  ...
  check(ctx) {
    ...
    function checkConditionMap(obj: Record<string, unknown>, path: string): void {
      const keys = Object.keys(obj);
      if (!keys.includes("types")) return;
      if (keys[0] !== "types") {
        results.push({
          severity: "error",
      ...
      for (const key of keys) {
        ...
        if (typeof val === "object" && val !== null && !Array.isArray(val)) {
          ...
          if (!nestedKeys.some((k) => k.startsWith("./"))) {
            checkConditionMap(nested, `${path}.${key}`);
          }
        }
      }
    }
    ...
    if (hasSubpaths) {
      for (const [subpath, val] of Object.entries(exports)) {
        if (typeof val === "object" && val !== null && !Array.isArray(val)) {
          checkConditionMap(val as Record<string, unknown>, `exports["${subpath}"]`);
        }
      }
    } else {
      // Sugar form — top-level is the condition map
      checkConditionMap(exports, 'exports["."]');
    }
