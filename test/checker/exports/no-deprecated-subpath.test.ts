...
describe("exports/no-deprecated-subpath", () => {
  it("warns on trailing / in exports keys", async () => {
    const results = await noDeprecatedSubpathRule.check({ ...baseCtx, pkg: { exports: { ".": "./dist/index.js", "./utils/": "./dist/utils/" } } });
    expect(results).toHaveLength(1);
    expect(results[0]!.message).toContain("deprecated");
  });

  it("passes with /* pattern", async () => {
    const results = await noDeprecatedSubpathRule.check({ ...baseCtx, pkg: { exports: { ".": "./dist/index.js", "./utils/*": "./dist/utils/*" } } });
    expect(results).toHaveLength(0);
  });

  it("skips non-object exports", async () => {
    const results = await noDeprecatedSubpathRule.check({ ...baseCtx, pkg: { exports: "./dist/index.js" } });
    expect(results).toHaveLength(0);
  });
});
