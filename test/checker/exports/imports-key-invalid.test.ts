...
describe("exports/imports-key-invalid", () => {
  it("errors on keys not starting with #", async () => {
    const results = await importsKeyInvalidRule.check({ ...baseCtx, pkg: { imports: { "lodash": "./shim.js", "#internal": "./src/internal.js" } } });
    expect(results).toHaveLength(1);
    expect(results[0]!.message).toContain("lodash");
  });

  it("passes with valid # keys", async () => {
    const results = await importsKeyInvalidRule.check({ ...baseCtx, pkg: { imports: { "#utils": "./src/utils.js" } } });
    expect(results).toHaveLength(0);
  });

  it("skips when no imports field", async () => {
    const results = await importsKeyInvalidRule.check({ ...baseCtx, pkg: {} });
    expect(results).toHaveLength(0);
  });
});
