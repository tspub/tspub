...
describe("files/implicit-index-format", () => {
  it("warns when index.js is ESM but package type is commonjs", async () => {
    vi.mocked(readFileSafe).mockReturnValue("export const x = 1;");
    vi.mocked(getCodeFormat).mockReturnValue("esm");
    const results = await implicitIndexFormatRule.check({
      ...baseCtx,
      pkg: {},
    });
  ...
  it("warns when index.js is CJS but package type is module", async () => {
  ...
  it("passes when index.js matches package type", async () => {
    vi.mocked(readFileSafe).mockReturnValue("module.exports = {};");
    vi.mocked(getCodeFormat).mockReturnValue("cjs");
    const results = await implicitIndexFormatRule.check({
      ...baseCtx,
  ...
  it("skips when main field exists", async () => {
    const results = await implicitIndexFormatRule.check({
      ...baseCtx,
  ...
  it("skips when exports field exists", async () => {
    const results = await implicitIndexFormatRule.check({
      ...baseCtx,
  ...
  it("skips when index.js not found", async () => {
    vi.mocked(readFileSafe).mockReturnValue(null);
    const results = await implicitIndexFormatRule.check({
      ...baseCtx,
      pkg: {},
