...
import type {
  ...
  CheckContext,
  FixContext,
} from "../../../src/checker/framework/types.js";
...
function buildContext(overrides: Partial<CheckContext> = {}): CheckContext {
  return {
    pkg: {},
    dir: "/tmp/test",
...
describe("checker framework runner", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe("runRules", () => {
    it("runs rules and collects diagnostics", async () => {
      ...
      const result = await runRules([mockRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: false,
      });
    ...
    it("previews fix without mutating pkg in dryRun mode", async () => {
      const mockRule: Rule = {
        ...
        fix: vi.fn().mockImplementation((ctx: FixContext) => {
    ...
    it("runs unsafe fix only when unsafe=true", async () => {
      ...
      const result1 = await runRules([mockRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: true,
      });
      ...
      const result2 = await runRules([mockRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: true,
        unsafe: true,
    ...
    it("filters fixes by fixTypes category", async () => {
      const matchingRule: Rule = {
        meta: {
          id: "test/matching-rule",
          description: "test",
          defaultSeverity: "warning",
          fixable: "safe" as const,
          category: "exports",
      ...
      const ctx = buildContext();
      const pkg: PackageJson = {};
      const result = await runRules([matchingRule, nonMatchingRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: true,
        fixTypes: ["exports"],
      ...
      expect(matchingRule.fix).toHaveBeenCalled();
      expect(nonMatchingRule.fix).not.toHaveBeenCalled();
      expect(result.fixes).toHaveLength(1);
      expect(result.fixes[0]?.message).toBe("exports fix");
    });
    ...
    it("accumulates diagnostics and fixes from multiple rules", async () => {
      ...
      const ctx = buildContext();
      const pkg: PackageJson = {};
      const result = await runRules([rule1, rule2, rule3], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: true,
      });
    ...
    it("captures fix error in dryRun mode as error diagnostic", async () => {
      ...
      expect(result.fixes).toHaveLength(0);
      // The original diagnostic + the fix error diagnostic
    ...
    it("passes distFiles to fix context", async () => {
      const fixSpy = vi.fn().mockReturnValue({
        message: "fixed with distFiles",
        pkgModified: false,
      });

      const mockRule: Rule = {
        meta: {
          id: "test/distfiles-rule",
          description: "test",
          defaultSeverity: "warning",
          fixable: "safe" as const,
          category: "test",
      ...
      await runRules([mockRule], ctx, {
        ...
        pkg,
        fix: true,
        distFiles: ["dist/index.js", "dist/types.d.ts"],
      });
    ...
    it("defaults distFiles to empty array when not provided in fix context", async () => {
      const fixSpy = vi.fn().mockReturnValue({
        message: "fixed",
        pkgModified: false,
      });
    ...
    it("skips rule with 'off' but continues running other rules", async () => {
      ...
      const ctx = buildContext();
      const pkg: PackageJson = {};
      const result = await runRules([skippedRule, keptRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: false,
    ...
    it("continues after a throwing rule and processes remaining rules", async () => {
      ...
      const ctx = buildContext();
      const pkg: PackageJson = {};
      const result = await runRules([throwingRule, normalRule], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: false,
      });
    ...
    describe("interactive mode", () => {
      ...
      it("does not apply fix when confirmed but pkgModified is false", async () => {
        const { createInterface } = await import("node:readline/promises");
        const { writePackageJson } = await import(
          "../../../src/shared/package-json.js"
        );
      ...
      it("skips fix with empty preview message in interactive mode", async () => {
        ...
        // The question should NOT have been asked since message was empty
    ...
    it("does not write package.json when no fix modifies pkg even with multiple rules", async () => {
      ...
      const ctx = buildContext();
      const pkg: PackageJson = {};
      await runRules([rule1, rule2], ctx, {
        dir: "/tmp/test",
        pkg,
        fix: true,
      });
    ...
    it("passes distFiles to fix in dryRun mode", async () => {
      const fixSpy = vi.fn().mockReturnValue({
        message: "dry fix",
        pkgModified: false,
      });
